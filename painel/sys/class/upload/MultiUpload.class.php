<?php
/**********************************************************************************************************************************************************
  Fazer multiplo upload de imagens
			...
			if(move_uploaded_file($this->nomeTemp,$this->pasta.$this->nome)){
			 ...
			 $ext = '.jpg';
				...
				if ($img) {
					...
					*/
					$this->inserir($nome.$ext,$this->idPagina);

					flush();
					
					/*Para redimensionar a mesma imagem, mais de 1 vez seguida, é preciso estar em diretórios diferentes*/
					$this->criar_miniatura($img,160,$dir.'imagem/miniatura/'.$nome);
					$this->criar_miniatura($img,1300,$dir.'imagem/original/'.$nome);
...
/*********************************************************************************************************************/ 
public function inserir($foto,$idPagina){
	   
	  $in = mysql_query("INSERT INTO imagem SET id_pagina='$idPagina',imagem='$foto' ") or die(mysql_error()); 
...
/*********************************************************************************************************************/ 
public function listarImagem($idPagina){
	
 $sql = mysql_query("SELECT id_imagem, imagem FROM imagem WHERE id_pagina='$idPagina' ")or die(mysql_error());
...
/*********************************************************************************************************************/ 
public function apagarImagem($idImagem,$idPagina){

 $sqlNome = mysql_query("SELECT imagem FROM imagem WHERE id_imagem='$idImagem' ");
 $sql     = mysql_query("DELETE FROM imagem WHERE id_imagem='$idImagem' ")or die(mysql_error());
...
public function apagarFotoDaPasta($sql){
 ...
 $pasta1 = 'uploads/imagem/miniatura/';
 $pasta2 = 'uploads/imagem/original/';
