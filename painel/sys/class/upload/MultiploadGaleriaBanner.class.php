...
/**********************************************************************************************************************************************************
  ...
  @data: 09/06/2014
**********************************************************************************************************************************************************/
  class MultiuploadGaleriaBanner{
			...
			if(move_uploaded_file($this->nomeTemp,$this->pasta.$this->nome)){
			 ...
			 $ext = '.jpg';
				...
				if ($img) {
					...
					*/
					$this->inserir($nome.$ext,$this->idBanner);

					flush();
					
					/*Para redimensionar a mesma imagem, mais de 1 vez seguida, é preciso estar em diretórios diferentes*/
					$this->criar_miniatura($img,160,$dir.'banner/miniatura/'.$nome);
					$this->criar_miniatura($img,1300,$dir.'banner/original/'.$nome);
...
/*********************************************************************************************************************/ 
public function inserir($foto,$idBanner){
	   
	  $in = mysql_query("INSERT INTO banner_galeria SET id_banner='$idBanner', imagem='$foto' ") or die(mysql_error()); 
...
/*********************************************************************************************************************/ 
public function listarImagem($idBanner){
	
 $sql = mysql_query("SELECT id_imagem, imagem FROM banner_galeria WHERE id_banner='$idBanner' ")or die(mysql_error());
...
/*********************************************************************************************************************/ 
public function apagarImagem($idImagem,$idBanner){

 $sqlNome = mysql_query("SELECT imagem FROM banner_galeria WHERE id_imagem='$idImagem' ");
 $sql     = mysql_query("DELETE FROM banner_galeria WHERE id_imagem='$idImagem' ")or die(mysql_error());
 
  if($sql==true){
	  
	$this->apagarFotoDaPasta($sqlNome);
    echo "<script>window.location.replace('?pag=EditarGaleriaBanner&id_banner=$idBanner')</script>";
...
public function apagarFotoDaPasta($sql){
 ...
 $pasta1 = 'uploads/banner/miniatura/';
 $pasta2 = 'uploads/banner/original/';
