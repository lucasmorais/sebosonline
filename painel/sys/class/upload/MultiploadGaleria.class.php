<?php
/**********************************************************************************************************************
  Classe responsável Fazer multiplo upload de produtos
  @autor: Marcelo Cavassani
  @data: 09/06/2014
**********************************************************************************************************************/
  class MultiuploadGaleria{
...
/*********************************************************************************************************************/
	...
	//conta a quantidade de arquivos que vieram do formulário
			...
			if(move_uploaded_file($this->nomeTemp,$this->pasta.$this->nome)){
			 ...
			 $ext = '.jpg';
				...
				if ($img) {
					...
					*/
					$this->inserir($nome.$ext,$this->idAlbum);

					flush();
					
					/*Para redimensionar a mesma imagem, mais de 1 vez seguida, é preciso estar em diretórios diferentes*/
					$this->criar_miniatura($img,160,$dir.'galeria/miniatura/'.$nome);
					$this->criar_miniatura($img,1300,$dir.'galeria/original/'.$nome);
...
/*********************************************************************************************************************/ 
public function inserir($foto,$idAlbum){
	   
	  $in = mysql_query("INSERT INTO galeria_foto SET id_album='$idAlbum',imagem='$foto' ") or die(mysql_error()); 
...
/*********************************************************************************************************************/ 
public function listarImagem($idAlbum){
	
 $sql = mysql_query("SELECT id_imagem, imagem FROM galeria_foto WHERE id_album='$idAlbum' ")or die(mysql_error());
...
/*********************************************************************************************************************/ 
public function apagarImagem($idImagem,$idAlbum){

 $sqlNome = mysql_query("SELECT imagem FROM galeria_foto WHERE id_imagem='$idImagem' ");
 $sql     = mysql_query("DELETE FROM galeria_foto WHERE id_imagem='$idImagem' ")or die(mysql_error());
 
  if($sql==true){
	  
	$this->apagarFotoDaPasta($sqlNome);
    echo "<script>window.location.replace('?pag=EditarGaleria&id_album=$idAlbum')</script>";
...
public function apagarFotoDaPasta($sql){
 ...
 $pasta1 = 'uploads/galeria/miniatura/';
 $pasta2 = 'uploads/galeria/original/';
